...
export const filterAffairs = (affairs: AffairType[], filter: FilterType): AffairType[] => {
  let filteredAffairs = affairs.map((el) => ({ ...el }));
  if (filter === "all") {
    return filteredAffairs;
  }

  return filteredAffairs.filter((el) => el.priority === filter);
};
export const deleteAffair = (affairs: AffairType[], _id: number) => {
  let copyAffairs = affairs.map((el) => ({ ...el }));

  return copyAffairs.filter((el) => el._id !== _id);
};
